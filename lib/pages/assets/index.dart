                  ...
                  builder: (_) {
                    ...
                    if (widget.service.plugin.basic.name == 'acala') {
                      if (widget.service.buildTarget != BuildTargets.dev) {
                        ...
                        if (widget
                                .service.store.settings.liveModules['assets'] !=
                            null) {
                    ...
                    if (widget.service.plugin.basic.name == 'karura') {
                      if (widget.service.buildTarget != BuildTargets.dev) {
                        if (widget
                                .service.store.settings.liveModules['claim'] !=
                            null) {
                          claimKarEnabled = widget.service.store.settings
                              .liveModules['claim']['enabled'];
                        }
                      } else {
                    ...
                    final symbol =
                        (widget.service.plugin.networkState.tokenSymbol ??
                            [''])[0];
                    final decimals =
                        (widget.service.plugin.networkState.tokenDecimals ??
                            [12])[0];

                    final balancesInfo = widget.service.plugin.balances.native;
                    final tokens =
                        widget.service.plugin.balances.tokens.toList();
                    final tokensAll =
                        widget.service.plugin.noneNativeTokensAll ?? [];

                    // add custom assets from user's config & tokensAll
                    final customTokensConfig =
                        widget.service.store.assets.customAssets;
                    if (customTokensConfig.keys.length > 0) {
                      ...
                      tokensAll.forEach((e) {
                        if (tokens.indexWhere((token) => token.id == e.id) <
                            0) {
                          tokens.add(e);
                        }
                      });
                    ...
                    final extraTokens =
                        widget.service.plugin.balances.extraTokens;
                    final isTokensFromCache =
                        widget.service.plugin.balances.isTokensFromCache;

                    String tokenPrice;
                    if (widget.service.store.assets.marketPrices[symbol] !=
                            null &&
                        balancesInfo != null) {
                      ...
                      appBar: AppBar(
                        ...
                        actions: <Widget>[
                          IconButton(
                            ...
                            onPressed: widget
                                        .service.keyring.allAccounts.length >
                                    0
                                ? () async {
                                    final selected =
                                        (await Navigator.of(context).pushNamed(
                                                NetworkSelectPage.route))
